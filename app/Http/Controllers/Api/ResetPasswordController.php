...
class ResetPasswordController extends Controller
{
    ...
    public function forget(ForgetPasswordRequest $request)
    {
        ...
        $response = Http::get('https://www.hisms.ws/api.php', $data = [
            ...
            'numbers' => $user->phone,
            'sender' => $sender,
          'message' => "كود اعادة التفغيل" .     $resetPasswordCode->code,
    ...
    public function code(ResetPasswordCodeRequest $request)
    {
        ...
        if (! $resetPasswordCode || $resetPasswordCode->isExpired() || ! $user) {
            throw ValidationException::withMessages([
                'code' => [
                    trans('validation.exists', ['attribute' => trans('auth.attributes.code')]),
                ],
            ]);
        }
    ...
    public function reset(ResetPasswordRequest $request)
    {
        ...
        if (! $resetPasswordToken || $resetPasswordToken->isExpired()) {
            throw ValidationException::withMessages([
                'token' => [
                    trans('validation.exists', ['attribute' => trans('auth.attributes.token')]),
                ],
            ]);
        }
